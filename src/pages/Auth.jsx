...
const Auth = () => {
  ...
  if (isSignedIn == undefined) {
    return (
      <div className="h-screen bg-gray-700 flex items-center justify-center">
        <div class="mx-auto w-[600px] bg-gray-950 rounded-xl overflow-hidden drop-shadow-2xl">
          <div class="bg-[#202020] flex items-center p-[20px] text-white relative rounded-t-xl">
            <div class="flex absolute left-3 space-x-2">
              <span class="h-3.5 w-3.5 bg-[#ff605c] rounded-full transition-all hover:scale-125 hover:bg-[#ff3b36]"></span>
              <span class="h-3.5 w-3.5 bg-[#ffbd44] rounded-full transition-all hover:scale-125 hover:bg-[#ffaa33]"></span>
              <span class="h-3.5 w-3.5 bg-[#00ca4e] rounded-full transition-all hover:scale-125 hover:bg-[#00b44e]"></span>
            </div>

            <div class="flex-1 text-center text-white font-semibold text-lg relative animate-pulse">
  ...
  const syncUser = async () => {
    api.post("/api/users/sync-user").then((data) => {
      console.log(data);
    });
  };
